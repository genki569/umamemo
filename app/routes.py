...
def save_race_memo(race_id):
    try:
        ...
        memo = RaceMemo(race_id=race_id, content=content)
        db.session.add(memo)
        db.session.commit()
        ...
        if form.validate_on_submit():
            try:
                review = RaceReview(
                    ...
                    price=int(request.form.get('price', 0)) if request.form.get('sale_status') == 'paid' else 0,
                ...
                db.session.rollback()
                app.logger.error(f"Error in race_review: {str(e)}")
                flash('ビューの作に失敗しました', 'error')
    
    return render_template('review/create.html', form=form, race_id=race_id)
...
@app.route('/race/<int:race_id>/review/<int:review_id>/purchase', methods=['GET', 'POST'])
@login_required
